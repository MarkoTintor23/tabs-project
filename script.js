class TabsProject {
  ...
  addEventListeners() {
    window.addEventListener("DOMContentLoaded", this.activeTab1.bind(this));
  ...
  buttonHandler(event) {
    ...
    const tabData = this.tabsArray.find((tab) => tab.id === tabId);

    if (tabData) {
      this.tabContent.textContent = tabData.text;
    }
  }

  activeTab1() {
    const firstTab = this.tabsArray[0];
    const firstButton = document.querySelector(`[data-id="${firstTab.id}"]`);

    if (firstButton) {
      firstButton.classList.add("active");
    }
    this.tabContent.textContent = firstTab.text;
  }
}

const tabsss = new TabsProject();
